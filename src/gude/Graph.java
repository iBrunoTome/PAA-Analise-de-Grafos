...
public class Graph implements Serializable {
	 ...
	 * @see #createVertex()
	 */
	public Edge createEdge(int alpha, int omega) {
		if (idVertexExists(alpha) && idVertexExists(omega))
			return new Edge(alpha, omega, this);
		else
			return null;
	}
	...
	public static void txt2xml(String nomeArquivo) {
		try {
			BufferedWriter arquivoXML = new BufferedWriter(new FileWriter(nomeArquivo.substring(0, nomeArquivo.length() - 4) + ".xml"));
			...
			while (linha != null) {
				...
				if (aux[0].equals("p")) {
					...
					arquivoXML.append("\n\t\t<Edge id=\"" + vertexAtual + "\" alpha=\"" + aux[1] + "\" omega=\"" + aux[2] + "\" />");
					vertexAtual++;
				}
